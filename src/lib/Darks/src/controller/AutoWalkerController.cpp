...
namespace Darks::Controller {
	AutoWalkerController::AutoWalkerController(
		...
		IO::GlobalKeyListener& global_keyboard_listener
	) :
		conf_(conf),
	...
	void AutoWalkerController::DisplayHUD() {
		if (running_) {
			ImGui::GetBackgroundDrawList()->AddText({ 20, 800 }, ImColor(255, 255, 255), "Auto Walk");
		}
	}
	...
	void AutoWalkerController::HandleKeystroke(IO::Key key) {
