...
namespace LXD::Controller {
	...
	bool BerryFarmerController::StopTimer(Darks::GlobalTimerManager& timer_manager) {
		if (!timer_id_) {
		...
		try {
			...
			for (int harvest_iteration = 0; harvest_iteration < 2; harvest_iteration++) {
				...
				for (int col = 0; col < len; col++) {
					...
					// mouse_controller_.Click(pos.x, pos.y + 500);
					// Look all the way up to harvest berries above
					camera_controller_.Pivot(info, Darks::Controller::CameraDirection::Up, 3000);
					// Must crouch to fit
					...
					if (harvest_iteration == 0) {
						...
						movement_controller_.Move(info, Darks::Controller::Movement::Forward, 6000); // ----------------- Change this if second pass starts too late or early
					}
					...
					// Replaces tintos with mejos:
					// This code will distribute mejos evenly between both the originally tintos row and mejos
					if (col == 0) {
