...
namespace LXD::Controller {
	...
	void FloraFarmer::WalkToCropFridge(Darks::SyncInfo& info, int fridge_index) const {
		...
		for (int i = 0; i < fridge_index; i++) {
			movement_controller_.Move(info, Darks::Controller::Movement::Forward, 3000);
			// Prevent the move left operation on the last iteration as the character
			// is already in front of the target fridge
			if (i < fridge_index - 1)
				movement_controller_.Move(info, Darks::Controller::Movement::Left, 2000);
		}
	}

	void FloraFarmer::HarvestCrops(Darks::SyncInfo& info, int iterations, std::string crop_search_str) const {
		// Iterate over crops harvesting
		for (int i = 0; i < iterations; i++) {
			// Move forward incrementally to harvest new crops
			movement_controller_.Move(info, Darks::Controller::Movement::Forward, 175);
			...
			if (!inventory_controller_.Close(info, Darks::Controller::Inventory::Other)) {
				...
				DARKS_ERROR(msg);
				throw Darks::AutonomousResetRequiredException(msg.c_str());
			}
		}
	}
	void FloraFarmer::TravelToFridges(Darks::SyncInfo& info) const {
		// Walk backwards all the way to the beds
		movement_controller_.Move(info, Darks::Controller::Movement::Backwards, 35000); // 20 seconds
		info.Wait(200);
		// Stop crouching
		movement_controller_.Crouch();
		info.Wait(500);
		// Walk left to the out teleporter
		movement_controller_.Move(info, Darks::Controller::Movement::Left, 7000);
		// Sit in the chair	
		...
		keyboard_controller_.Keystroke(Darks::IO::Key::Z);
		info.Wait(10000);
		// Move to the right to align with chair
		movement_controller_.Move(info, Darks::Controller::Movement::Right, 1500);
	...
	void FloraFarmer::DepositCropsInFridges(Darks::SyncInfo& info, int crop_type, int fridge_iterations) const {
		for (int fridge_index = 0; fridge_index < fridge_iterations; fridge_index++) {
			...
			movement_controller_.Move(info, Darks::Controller::Movement::Right, 350);
		}
