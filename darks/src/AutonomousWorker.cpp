...
namespace Darks {
	void AutonomousWorker::Start() {
		...
		queue_info_ = std::make_unique<Controllers::QueueSyncInfo>(
			&work_queue_,
			work_queue_mutex_,
		...
		*/
		...
		thread_ = std::thread([this]() {
			...
			Controllers::IQueueable* task = nullptr;
			...
			do { // the main loop for handling exceptions and attempting to return to a operatable state
				try {
					...
					if (!spawn_controller_.IsSpawnScreenOpen(Controllers::SpawnScreen::DeathScreen)) {
					...
					do { // the queue loop for processing jobs
						...
						if (!work_queue_.empty())
						{
							...
							if (auto* specifier = dynamic_cast<Controllers::ISpecifyServer*>(task)) {
								if (std::string target_serv = specifier->GetServer(); current_server_ != specifier->GetServer()) {
									...
									if (!spawn_controller_.IsSpawnScreenOpen(Controllers::DeathScreen)) {
