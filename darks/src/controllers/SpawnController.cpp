...
namespace Darks::Controllers {
	...
	bool SpawnController::Clear(SyncInfo& info) const {
		...
		// Highlight all text inside searchbar and delete it
		keyboard_controller_.Keystroke(IO::Key::LeftCtrl, KeyState::Pressed);
		keyboard_controller_.Keystroke(IO::Key::A);
		keyboard_controller_.Keystroke(IO::Key::LeftCtrl, KeyState::Released);
		keyboard_controller_.Keystroke(IO::Key::BackSpace);
		return true;
	}
