...
static std::optional<Key> ParseKeyStr(std::string key_str) {
	...
	if (!initialized) {
		...
		key_map.insert({ "`", Key::OEMBackTab });
		key_map.insert({ "\\",Key::OEM5 });
		key_map.insert({ "[", Key::OEM4 });
		key_map.insert({ "]", Key::OEM6 });
		key_map.insert({ ",", Key::Comma });
		key_map.insert({ "0", Key::Num0 });
		key_map.insert({ "1", Key::Num1 });
		key_map.insert({ "2", Key::Num2 });
		key_map.insert({ "3", Key::Num3 });
		key_map.insert({ "4", Key::Num4 });
		key_map.insert({ "5", Key::Num5 });
		key_map.insert({ "6", Key::Num6 });
		key_map.insert({ "7", Key::Num7 });
		key_map.insert({ "8", Key::Num8 });
		key_map.insert({ "9", Key::Num9 });
		key_map.insert({ "W", Key::W });
		key_map.insert({ "S", Key::S });
	}

	if (auto search = key_map.find(key_str); search != key_map.end()) {
		return std::optional<Key>(search->second);
	}
